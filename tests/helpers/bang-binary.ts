...
export const BANG_BINARY_VERSION = 7;
...
export function bangBinaryTriggerLengthsOffset(header: Uint32Array): number {
  return BANG_BINARY_HEADER_WORDS * U32_BYTES + header[3] * header[12];
}

export function bangBinaryTriggerLengths(
  binary: ArrayBuffer,
  header: Uint32Array
): Uint8Array | Uint16Array {
  return header[4] === 1
    ? new Uint8Array(binary, bangBinaryTriggerLengthsOffset(header), header[2])
    : new Uint16Array(
        binary,
        bangBinaryTriggerLengthsOffset(header),
        header[2]
      );
}

export function bangBinaryTriggerLocalOffsets(
  binary: ArrayBuffer,
  header: Uint32Array,
  triggerLengths: Uint8Array | Uint16Array = bangBinaryTriggerLengths(
    binary,
    header
  )
): Uint8Array {
  return new Uint8Array(
    binary,
    bangBinaryTriggerLengthsOffset(header) + triggerLengths.byteLength,
