...
import {
  ...
  originOfPrefix,
  parsePrefixBang,
  type TriggerHashResult,
  trimRawEnd,
  trimRawStart,
  type UrlParts,
} from "./redirect-prefix";

export type { UrlParts } from "./redirect-prefix";

export function compileTriggerMarker(code: number): number {
  return compileMarker(code);
}
...
const triggerHashScratch: TriggerHashResult = { hash: 0, trigger: "" };
...
function extractTrigger(s: string, from: number, to: number): string {
  extractTriggerWithHash(s, from, to, triggerHashScratch);
  _lastHash = triggerHashScratch.hash;
  return triggerHashScratch.trigger;
}
...
function resolveRaw(
  ...
  if (exclStart !== -1) {
    const parsed = parsePrefixBang(rawQuery, exclStart + exclWidth, end);
    if (parsed.kind === "home") {
      return "/";
    }
    ...
    _lastHash = parsed.hash;
    if (parsed.termStart === null) {
      const origin = resolveBangOrigin(parsed.trigger, custom, parsed.hash);
      if (!origin) {
        return buildUrl(defaultUrl, rawQuery, start, end);
      }
      return resolveWithTrigger(origin, parsed.trigger);
    }
    ...
    return resolveWithTrigger(filled, parsed.trigger);
  }
...
export function redirectUrlResult(
  query: string,
  settings: RedirectSettings,
  hotBangLookup?: HotBangLookup | null
): readonly [url: string, trigger: string | null] {
  const url = resolveRaw(encodeForRedirect(query), settings, hotBangLookup);
  return [url, _resolvedTrigger];
}
