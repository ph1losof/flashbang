...
│   ├── bang-shard/
│   │   └── [version]/
│   │       └── [shard].ts    # Cloudflare materialized-shard endpoint
│   ├── suggest.ts            # Cloudflare Pages Function for /suggest
│   └── opensearch.xml.ts     # Cloudflare Pages Function for /opensearch.xml
├── scripts/
│   ├── benchmark-shard-endpoint.ts # Remote first-pass/cached shard benchmark
...
│   ├── server/
│   │   ├── bang-shard.ts     # Host-neutral immutable shard response
│   │   ├── cloudflare-bang-shard.ts # Pages asset/cache adapter
│   │   ├── handlers.ts       # Production server request handlers
│   │   └── headers.ts        # CSP and security headers (shared across all targets)
│   ├── shared/
│   │   ├── bang-shard-pack.ts # Global-string pack and shard materializer
...
After deploying a preview, benchmark the experimental endpoint without changing client traffic:

```sh
bun run benchmark:shards -- https://preview.example.com <12-hex-pack-version> 5
```

The version is the suffix in `dist/bangs-pack-<version>.bin`. The report separates the first request for every shard from repeated cached passes and includes the endpoint's cache hit/miss marker.
