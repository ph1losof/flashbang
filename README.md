...
| -------------------------------- | ------------------------------------------------------- | ------------------------------------------------- | ----------------------------------------------------------------- | ------------------------------------------------------ | ---------------------------------------------------------- | ---------------------------------------------------------- |
| **Redirect method**              | Service Worker intercept                                | `window.location.replace`                         | `window.location.replace`                                         | Cloudflare Worker (edge) + client fallback             | `window.location.replace`                                  | Cloudflare Worker (edge) + client fallback                 |
| **When redirect happens**        | Before page renders (Service Worker)                    | After page loads (HTML, CSS, JS)                  | After page loads (HTML, CSS, JS)                                  | At the edge or after page loads (HTML, CSS, JS, React) | After page loads (HTML, CSS, JS, React)                    | At the edge or after page loads (HTML, CSS, JS, React)     |
| **Sources**                      | DDG + Kagi + custom                                     | DDG                                               | Kagi                                                              | DDG + Kagi                                             | Selectable DDG or Kagi datasets                            | DDG + Kagi                                                 |
| **Analytics**                    | None†                                                   | Plausible                                         | None‡                                                             | Vercel Analytics + Speed Insights                      | None                                                       | None                                                       |
| **Server required**              | No (redirects); yes (suggestions, OpenSearch)           | No                                                | No                                                                | Yes (Cloudflare Worker)                                | No                                                         | Yes (Cloudflare Worker)                                    |
...
| **Address-bar suggestions**      | Built in (bang-aware + regular queries)                 | No                                                | Manual Google/DDG provider URL only                               | No                                                     | No                                                         | Regular DDG suggestions only                               |
| **Frecency-ranked suggestions**  | Yes (local usage; Chromium)                             | No                                                | No                                                                | No                                                     | No                                                         | No                                                         |
| **OpenSearch**                   | Yes (dynamic, self-host friendly)                       | No                                                | Yes                                                               | Yes                                                    | Yes (static)                                               | Yes                                                        |
| **Default engine setting**       | Yes                                                     | `localStorage` only; no UI                         | Yes                                                               | Yes                                                    | Yes                                                        | Yes                                                        |
| **Custom bangs**                 | Yes (IndexedDB)                                         | No                                                | Yes (`localStorage`)                                              | Yes (`localStorage`)                                   | Yes (IndexedDB)                                            | Yes (`localStorage` + cookie)                              |
...
| **Bang data for redirects**      | ~708 KB (packed trigger→URL lookup)                     | 2.7 MB (full metadata)                            | 1.5 MB (full metadata)                                            | ~208 KB inline + 1.23 MB lazy-loaded                  | Selected JSON dataset cached in IndexedDB                  | Edge map + full client catalog                             |
| **Parsed on**                    | SW thread (once per worker lifetime)                    | Main thread (every page load)                     | Main thread (every page load)                                     | Main thread (client fallback) or edge worker           | Main thread                                                | Main thread (client fallback) or edge worker               |
...
> **Note:** Feature data was checked against the linked projects' default branches on July 22, 2026. "No" means no built-in implementation was found; manual browser configuration is not counted. These projects are actively developed and may have changed since.
