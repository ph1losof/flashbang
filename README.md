...
| **Core client redirect assets¶** | 🥈 **First redirect: ~13.7 KiB hot / ~20.9 KiB other; complete offline: ~238 KiB transfer / ~622 KiB decoded**                                          | 🥉 ~466 KiB transfer / ~1,980 KiB decoded                                                                                                        | 🥇 **First redirect: ~1.3 KiB (edge 302); complete offline: ~191 KiB transfer / ~694 KiB decoded**                                                                                                                               | Edge path                                                                                                                                          | ~525 KiB transfer / ~2,295 KiB decoded                                                                                                                         | Edge path                                                                                                                                                     |
...
¶ This row ranks the network bytes required before the first redirect first, then the complete decoded redirect runtime; compressed bytes used only for background/offline warming are secondary. Flashbang reports both states because its sharded design does not put the complete offline catalog in front of the first redirect: a generated hot bang transfers the cold resolver in two requests (~13.7 KiB / ~31.4 KiB decoded), while another built-in adds one routed shard for ~20.9 KiB / ~38.6 KiB decoded across three requests. Its complete installed worker and warm offline catalog are ~238.3 KiB transferred across 20 requests. “Transfer” uses each deployment's supported HTTP content encoding; “decoded” is the body size after HTTP decompression. Other Service Worker totals include the installed worker and complete packed catalog but exclude their first-uncontrolled-navigation fallback. Page-based totals include the JavaScript and fetched catalog used by their normal client redirect. HTML, CSS, fonts, analytics, settings, suggestions, and unrelated UI assets are excluded. Unduckified ranks first on this row as of its August 21, 2026 edge-redirect release: a first search in a new profile is now answered by a Cloudflare Pages Function rather than by its client, so nothing but the redirect response itself precedes the destination request — a measured ~1.3 KiB in a single request, for both a hot bang and a full-catalog alias. Its first contact is an 837-byte bootstrap document that registers the Service Worker and forwards via `location.replace()`; every later uncontrolled search is a bare 302. Its installed client runtime is unchanged and still decodes larger than Flashbang's.
...
### Production benchmark: 7 wins, 2 ties, 2 losses

The August 21, 2026 campaign ran 60 randomized pairs in every category against the current public deployments, immediately after Unduckified shipped its edge redirect that same day. A category is decided when the bootstrapped 95% confidence interval on the paired mean difference excludes zero, which is stricter than counting pair majorities. Flashbang took seven categories — every live-worker and worker-restart category it did not tie — and Unduckified took both new-profile categories.

| Lifecycle      |              Score | **Flashbang median range** | **Unduckified median range** |
| -------------- | -----------------: | -------------------------: | ---------------------------: |
| Live worker    | **4 wins, 2 ties** |           **0.39–0.40 ms** |                 0.41–0.48 ms |
| Worker restart |         **3 wins** |           **3.11–3.13 ms** |                 3.66–3.94 ms |
| New profile    |       **2 losses** |               76.27–88.46 ms |         **60.81–61.50 ms** |

The live-worker margins remain small — 0.00–0.09 ms — and two of those six categories have confidence intervals spanning zero. The durable separation is the restart path: Flashbang was 0.48–1.26 ms lower after a stopped worker restarted, taking 59/60, 56/60, and 59/60 pairs at p < 0.0001. The new-profile result reversed. Unduckified now answers a first search from a Cloudflare Pages Function instead of shipping a catalog to the client, so it resolves that navigation in one round trip and ~1.3 KiB where Flashbang loads a cold resolver and, for a non-hot bang, one routed shard. It won 48/60 and 57/60 new-profile pairs by 9.81 ms and 34.38 ms. That row measures the one-time cold install: the single search a profile makes before a Service Worker exists, after which every subsequent search for the life of that profile takes the live-worker path at 0.39–0.40 ms or the restart path at 3.11–3.13 ms, both of which Flashbang wins. That one-off 9.81–34.38 ms is repaid after roughly 10–70 worker restarts at 0.48–1.26 ms each — a state a browser enters routinely whenever it stops an idle worker between searches — and is never paid again. It is also a deliberate abstention rather than a gap to close, because the saving is bought by resolving the search on a server — exactly what Flashbang's on-device design declines to do. The architecture behind these states, and the privacy tradeoff behind that row, is explained in [Why Flashbang is faster](#why-is-flashbang-faster).
...
| ---------------------------------------------- | ------------: | --------------: | ------------------------------------------------------------------------------------ |
| Live worker, `!gh test`                        |   **0.39 ms** |         0.41 ms | Tie — 45/15 pairs but mean difference 0.01 ms (95% CI -0.03–0.03, p = 0.9227)         |
| Live worker, full-catalog alias `!github test` |   **0.39 ms** |         0.41 ms | **Flashbang won 45/60; mean advantage 0.01 ms** (95% CI 0.01–0.02, p = 0.0004)        |
| Live worker, trailing `test !gh`               |   **0.39 ms** |         0.42 ms | **Flashbang won 46/60; mean advantage 0.02 ms** (95% CI 0.01–0.03, p < 0.0001)        |
| Live worker, suffix `test gh!`                 |   **0.40 ms** |         0.41 ms | Tie — 38/22 pairs but mean difference -0.00 ms (95% CI -0.05–0.02, p = 0.9965)        |
| Live worker, empty query `!gh`                 |   **0.40 ms** |         0.42 ms | **Flashbang won 41/60; mean advantage 0.01 ms** (95% CI 0.00–0.02, p = 0.0259)        |
| Live worker, 256-byte query                    |   **0.39 ms** |         0.48 ms | **Flashbang won 58/60; mean advantage 0.09 ms** (95% CI 0.08–0.10, p < 0.0001)        |
| Worker restart, `!gh test`                     |   **3.12 ms** |         3.66 ms | **Flashbang won 59/60; mean advantage 0.54 ms** (95% CI 0.48–0.60, p < 0.0001)        |
| Worker restart, full-catalog alias             |   **3.13 ms** |         3.70 ms | **Flashbang won 56/60; mean advantage 0.48 ms** (95% CI 0.32–0.60, p < 0.0001)        |
| Worker restart, 256-byte query                 |   **3.11 ms** |         3.94 ms | **Flashbang won 59/60; mean advantage 1.26 ms** (95% CI 0.87–1.73, p < 0.0001)        |
| New profile, `!gh test`                        |      76.27 ms |    **61.50 ms** | **Unduckified won 48/60; mean advantage 9.81 ms** (95% CI 2.05–16.46, p = 0.0077)     |
| New profile, full-catalog alias                |      88.46 ms |    **60.81 ms** | **Unduckified won 57/60; mean advantage 34.38 ms** (95% CI 23.96–50.42, p < 0.0001)   |

A category is awarded when the bootstrapped 95% confidence interval on the paired mean difference excludes zero. Pair counts are reported alongside because the two can disagree: `!gh test` and `test gh!` on a live worker go to Flashbang 45/15 and 38/22 by pair count, but their mean differences are 0.01 ms and -0.00 ms with intervals spanning zero, so both are recorded as ties rather than wins.

The comparison uses Unduckified's timing definition: CDP `Network.requestWillBeSent` wall time from navigation to the tool URL until the first GitHub request. It ran in Chromium 151 on an Apple M4 Max; discarded three warm-up pairs; randomized tool order; measured live workers, explicitly stopped workers, and isolated browser contexts; bootstrapped 95% confidence intervals; and used a paired sign-flip permutation test. Absolute medians drift between sittings — repeated campaigns on this machine moved Flashbang's live-worker median between 0.39 ms and 0.65 ms with no code change — so only within-campaign paired comparisons are reported here, and figures from different campaigns are not compared against each other.

The payload audit separates critical-path behavior from total offline footprint, and Unduckified now wins the critical path outright. Over 10 rounds per scenario on August 21, 2026, Unduckified transferred a median 1.29 KiB in a single request before the destination request started, identically for the hot bang `!gh test` and the full-catalog alias `!github test`, because neither is resolved on the client any more. Flashbang transferred 13.72 KiB across two requests for the hot bang (~31.4 KiB decoded) and 20.91 KiB across three for the alias (~38.6 KiB decoded), the third request being the routed shard. Flashbang's complete worker plus warm catalog measures 238.3 KiB transferred across 20 requests, and continues caching only after the destination request starts.
