...
<details>
<summary>Comparison notes, measurement definitions, and source revisions</summary>
...
Unduckified was rechecked at current `main` commit `58b33a7` on August 3, 2026; the remaining feature data was checked on July 23, 2026. "No" means no built-in implementation was found; manual browser configuration is not counted. These projects are actively developed and may have changed since.

</details>

### Flashbang vs Unduckified: 9 wins, 2 ties, 0 losses

The production campaign ran 60 randomized pairs per category against the current public deployments. Under its predeclared 5% per-category significance rule, Flashbang won every worker-restart and new-profile category; the only ties were two live-worker query forms.

| Lifecycle | Score | **flashbang median range** | **Unduckified median range** |
| --- | ---: | ---: | ---: |
| Live worker | **4 wins, 2 ties** | **0.43–0.47 ms** | 0.45–0.55 ms |
| Worker restart | **3 wins** | **2.88–2.93 ms** | 4.00–4.04 ms |
| New profile | **2 wins** | **96.25–106.76 ms** | 156.23–163.28 ms |

The live-worker wins are only 0.02–0.08 ms; the more useful differences are 1.08–1.14 ms after a stopped worker restarts and 47.76–96.12 ms on the measured first search. The architecture behind those states is explained in [Why is it faster?](#why-is-it-faster).
...
These numbers were measured on August 3, 2026 against `flashbang.tech` and Unduckified's public `s.dunkirk.sh` deployment, which was serving the v9 catalog format introduced by current `main` commit [`58b33a7`](https://github.com/taciturnaxolotl/unduckified/commit/58b33a7004472724a2ba76e612af549049cff2fa). Unduckified's [checked-in benchmark](https://github.com/taciturnaxolotl/unduckified/blob/58b33a7004472724a2ba76e612af549049cff2fa/bench/redirect-bench.mjs) still points to an older preview serving v7, so that stale preview was not used. The comparison otherwise uses Unduckified's timing definition—CDP `Network.requestWillBeSent` wall time from the tool navigation to the first GitHub request.

The benchmark ran in Chromium 151 on an Apple M4 Max; measured live workers, explicitly stopped workers, and isolated new browser contexts; covered leading, trailing, suffix, bare, long-query, hot-tier, and full-catalog inputs; discarded three warm-up pairs; randomized tool order; bootstrapped 95% confidence intervals; and ran a paired sign-flip permutation test. [Raw samples and generated summaries](bench/results/unduckified-live-comparison-2026-08-03-rerun.json) are archived with the repository.

</details>

## Why is it faster?

### Page and edge redirect paths
