...
These numbers were measured on August 3, 2026 against `flashbang.tech` and Unduckified's public `s.dunkirk.sh` deployment, which was serving the v9 catalog format introduced by current `main` commit [`58b33a7`](https://github.com/taciturnaxolotl/unduckified/commit/58b33a7004472724a2ba76e612af549049cff2fa). The comparison uses CDP `Network.requestWillBeSent` wall time from the tool navigation to the first GitHub request.

The benchmark ran in Chromium 151 on an Apple M4 Max; measured live workers, explicitly stopped workers, and isolated new browser contexts; covered leading, trailing, suffix, bare, long-query, hot-tier, and full-catalog inputs; discarded three warm-up pairs; randomized tool order; bootstrapped 95% confidence intervals; and ran a paired sign-flip permutation test.
