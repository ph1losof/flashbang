...
- **Search suggestions** — The only bang tool with bang-aware autocomplete in your browser's native address bar. Type `!y` and the browser itself suggests `!yt` (YouTube), `!ya` (Yandex), `!yf` (Yahoo Finance) — ranked by a combination of global popularity and your personal usage frequency. Regular queries can use Google, DuckDuckGo, Bing, Brave, Yahoo, Ecosia, Kagi, Qwant, Startpage, Yandex, or Baidu. Opt-in site-specific forwarding asks the completed bang's own autocomplete service instead, so `!gh react` suggests GitHub repositories, `!wde berlin` suggests German Wikipedia pages, and package bangs can use AUR, Maven Central, NuGet, or Packagist. Modrinth and Mozilla Add-ons are supported too. Self-hosted deployments can explicitly opt into custom providers. Everything is unified through a single `/suggest` endpoint that plugs into your browser's built-in suggestion UI. Firefox-based browsers also render bang descriptions, site names, and favicons inline in the dropdown via `google:suggestdetail`. See [Browser quirks](#browser-quirks) for rendering and cookie differences across browsers
...
- `site_specific_forward=1` sends the non-bang terms to a completed built-in bang's own autocomplete endpoint when one is available
...
This option changes where suggestion text is sent. For example, text after `!gh` reaches GitHub's API and text after `!w` reaches Wikipedia. It is disabled unless the URL explicitly contains `site_specific_forward=1`; redirects remain local and are unaffected.
...
| -------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Redirect method**              | 🥇 **Service Worker intercept + client fallback**                                                                                                     | 🥉 **`window.location.replace`**                                                                                                                 | 🥇 **Service Worker intercept + client fallback**                                                                                                                         | 🥈 **Cloudflare Worker + client fallback**                                                                                                         | 🥉 **`window.location.replace`**                                                                                                                               | 🥈 **Cloudflare Worker + client fallback**                                                                                                                    |
...
| **Sources**                      | 🥇 **DDG + Kagi + custom**                                                                                                                            | DDG                                                                                                                                              | Kagi + custom                                                                                                                                                             | 🥈 **DDG + Kagi**                                                                                                                                  | 🥉 **Selectable DDG or Kagi datasets**                                                                                                                         | **Kagi**                                                                                                                                                      |
| **Source-measured bang triggers‖** | **14,614**                                                                                                                                          | Not measured                                                                                                                                     | **13,593**                                                                                                                                                                | Not measured                                                                                                                                       | Not measured                                                                                                                                                   | Not measured                                                                                                                                                  |
| **Analytics**                    | 🥇 **None†**                                                                                                                                          | Plausible                                                                                                                                        | Cloudflare `beacon.min.js` on hosted deployment‡                                                                                                                         | Vercel Analytics + Speed Insights                                                                                                                  | 🥇 **None (static GitHub Pages)**                                                                                                                              | Google Analytics 4 via Cloudflare Zaraz§                                                                                                                      |
...
| **Address-bar suggestions**      | 🥇 **Built in (bang-aware + regular queries + opt-in site forwarding)**                                                                               | No                                                                                                                                               | 🥈 **Built-in bang completion + opt-in provider/site forwarding**                                                                                                         | No                                                                                                                                                 | No                                                                                                                                                             | 🥈 **Built-in Google suggestions**                                                                                                                            |
...
| **Custom bangs**                 | 🥇 **Yes (IndexedDB + navigation-preload hot boot)**                                                                                                  | No                                                                                                                                               | 🥈 **Yes (`localStorage` + SW Cache Storage)**                                                                                                                            | 🥉 **Yes (`localStorage`)**                                                                                                                        | 🥈 **Yes (IndexedDB)**                                                                                                                                         | 🥉 **Yes (`localStorage` + cookie)**                                                                                                                          |
| **Advanced custom bangs**        | 🥇 **Regex captures, encoding modes, custom snap paths**                                                                                              | No                                                                                                                                               | No                                                                                                                                                                        | No                                                                                                                                                 | No                                                                                                                                                             | No                                                                                                                                                            |
| **Bang catalog search**          | Yes (trigger, name, or domain)                                                                                                                        | No                                                                                                                                               | No                                                                                                                                                                        | Yes                                                                                                                                                | Yes                                                                                                                                                            | Yes                                                                                                                                                           |
...
| **Bang data for redirects**      | 🥇 **~536 KiB packed trigger→URL lookup**                                                                                                             | 2.7 MB (full metadata)                                                                                                                           | 🥈 **~688 KiB packed trigger→URL lookup**                                                                                                                                 | 🥉 **~208 KB inline + 1.23 MB lazy-loaded**                                                                                                        | ~1,744–1,977 KiB JSON dataset                                                                                                                                  | ~1,560 KiB generated trigger map                                                                                                                              |
| **Redirect runtime payload¶**    | 🥇 **~217 KiB transfer / ~594 KiB decoded**                                                                                                           | 🥉 ~466 KiB transfer / ~1,980 KiB decoded                                                                                                        | 🥇 **~191 KiB transfer / ~694 KiB decoded**                                                                                                                     | 🥇 N/A                                                                                                                                             | ~525 KiB transfer / ~2,295 KiB decoded                                                                                                                         | 🥇 N/A                                                                                                                                                        |
...
‡ Unduckified's source contains no analytics code, but its hosted custom domain currently requests Cloudflare's `beacon.min.js` on both landing-page and query navigations. Issue [#13](https://github.com/taciturnaxolotl/unduckified/issues/13) documents the beacon remaining in browser network requests after the author disabled Web Analytics; the author suspected injection by Cloudflare's protection layer. Cloudflare [documents the beacon](https://developers.cloudflare.com/web-analytics/data-metrics/data-origin-and-collection/) as part of its Web Analytics collection path. This is deployment-level behavior rather than code in the Unduckified repository.
...
‖ Counts are built-in redirect triggers after source aliases and project-specific entries are expanded. Flashbang's `data/bangs.json` contains 14,614 triggers; Unduckified's pack input at commit `58b33a7` produces 13,593. Flashbang therefore covers 1,021 more triggers (7.5%). Unduckified's currently Brotli-compressed redirect runtime transfers 26 KiB (12%) less, while Flashbang remains 100 KiB (14%) smaller decoded.

¶ Redirect runtime payload is the minified client code plus catalog data needed to resolve a built-in bang. Flashbang and Unduckified were remeasured live on August 3, 2026; the remaining deployments were measured live on July 23, 2026. The first value is the response-body bytes transferred using each deployment's supported content encoding; the second is the decoded size. Unduckified's benchmark snapshot received an identity-encoded catalog, but immediate post-campaign checks consistently received the 704,519-byte `bangs.bin` as a 192,949-byte Brotli response on both cache misses and revalidated requests; the table uses that current delivery behavior. Service Worker tools include their installed worker and packed redirect index. Page-based tools include the JavaScript and fetched data used by their redirect path. HTML, CSS, fonts, analytics, settings, suggestions, and other non-redirect assets are excluded. ReBang and cf-unduck return an empty-body edge `302` for a supported built-in bang, so their successful edge path has no client runtime payload; their larger client fallback paths are not included.

> **Note:** Unduckified was rechecked at current `main` commit `58b33a7` on August 3, 2026; the remaining feature data was checked on July 23, 2026. "No" means no built-in implementation was found; manual browser configuration is not counted. These projects are actively developed and may have changed since.

### Flashbang vs Unduckified: measured lifecycle latency

In the production campaign below, Flashbang won **9 of 11 measured categories** under the benchmark's predeclared per-category 5% significance rule, with two statistical ties and no Unduckified wins: four of six live-worker scenarios, all three worker-restart scenarios, and both first-search scenarios. The live-worker advantage is statistically repeatable but only 0.02–0.08 ms in the representative rows below. The larger and more useful difference is the state Flashbang's hot boot is designed for: an **installed, cached profile whose Service Worker has been stopped by the browser**. On the next search, Unduckified reads and initializes its complete cached catalog before resolving the bang. Flashbang restores compact redirect state while the full catalog and IndexedDB state warm in the background.

| Chromium 151, randomized pairs | **flashbang** | **Unduckified** | Paired result |
| --- | ---: | ---: | --- |
| Live worker, `!gh test` | **0.43 ms** | 0.45 ms | **Flashbang mean advantage 0.02 ms** (95% CI 0.01–0.03, p = 0.0010) |
| Live worker, full-catalog alias `!github test` | **0.43 ms** | 0.45 ms | **Flashbang mean advantage 0.02 ms** (95% CI 0.01–0.03, p < 0.0001) |
| Live worker, 256-byte query | **0.47 ms** | 0.55 ms | **Flashbang mean advantage 0.08 ms** (95% CI 0.07–0.10, p < 0.0001) |
| Worker restart, `!gh test` | **2.88 ms** | 4.02 ms | **Flashbang won 60/60; mean advantage 1.08 ms** (95% CI 1.01–1.15, p < 0.0001) |
| Worker restart, full-catalog alias | **2.89 ms** | 4.00 ms | **Flashbang won 60/60; mean advantage 1.14 ms** (95% CI 1.07–1.22, p < 0.0001) |
| Worker restart, 256-byte query | **2.93 ms** | 4.04 ms | **Flashbang won 60/60; mean advantage 1.12 ms** (95% CI 1.05–1.19, p < 0.0001) |
| New profile, `!gh test` (60 pairs) | **106.76 ms** | 156.23 ms | **Flashbang mean advantage 47.76 ms** (95% CI 29.95–65.28, p < 0.0001) |
| New profile, full-catalog alias (60 pairs) | **96.25 ms** | 163.28 ms | **Flashbang mean advantage 96.12 ms** (95% CI 55.22–159.44, p < 0.0001) |

These numbers were measured on August 3, 2026 against `flashbang.tech` and Unduckified's public `s.dunkirk.sh` deployment, which was serving the v9 catalog format introduced by current `main` commit [`58b33a7`](https://github.com/taciturnaxolotl/unduckified/commit/58b33a7004472724a2ba76e612af549049cff2fa). Unduckified's [checked-in benchmark](https://github.com/taciturnaxolotl/unduckified/blob/58b33a7004472724a2ba76e612af549049cff2fa/bench/redirect-bench.mjs) still points to an older preview serving v7, so that stale preview was not used for this table. The comparison otherwise uses Unduckified's timing definition—CDP `Network.requestWillBeSent` wall time from the tool navigation to the first GitHub request. It ran 60 measured pairs per category in Chromium 151 on an Apple M4 Max; measured live workers, explicitly stopped workers, and isolated new browser contexts; covered leading, trailing, suffix, bare, long-query, hot-tier, and full-catalog inputs; discarded three warm-up pairs; randomized tool order; bootstrapped 95% confidence intervals; and ran a paired sign-flip permutation test. [Raw samples and generated summaries](bench/results/unduckified-live-comparison-2026-08-03-rerun.json) are archived with the repository.

For a first-ever search, Flashbang starts the selected deterministic ~15–18 KiB catalog shard and the cold resolver module in parallel, redirects as soon as that shard resolves, then lets the Service Worker warm the complete catalog for later offline redirects. That new-profile state is a network-sensitive end-to-end measurement—CDN delivery, first-page fallback, catalog transfer, parsing, and installation—not a Service Worker restart measurement. The current deployed campaign again favored Flashbang for both the hot `!gh` query and the full-catalog `!github` alias. Unduckified does not publish the raw samples or environment behind its earlier claimed cold `p < 0.001`, so that exact claim cannot be audited.

## Why is it faster?

Page-based bang tools such as unduck and bangs.fast pay the same unavoidable cost: your browser navigates to their site, loads HTML, parses and executes JavaScript, and only then calls `window.location.replace()` to send you somewhere else. You see the detour happen: a blank screen, a white flash, or their page flickering into view before the real destination appears. That is not the destination loading slowly; it is an entire intermediate application starting up just to throw itself away. It typically costs 100–500ms depending on the browser and device, and it happens on every redirect, even when the files are cached. bangs.fast improves how its catalog is cached, but the browser still enters its page and runs client JavaScript before it can redirect.
