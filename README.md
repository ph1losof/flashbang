...
| --------- | ----: | -------------------------: | ---------------------------: |
| Live worker | **6 wins** | **0.44–0.46 ms** | 0.47–0.54 ms |
| Worker restart | **3 wins** | **2.88–2.94 ms** | 3.85–4.08 ms |
| New profile | **2 wins** | **93.92–94.33 ms** | 146.67–151.63 ms |

The live-worker advantages are only 0.02–0.08 ms. The useful separation is 0.93–1.15 ms after a stopped worker restarts and 57.59–62.69 ms on the measured first search. The architecture behind those states is explained in [Why Flashbang is faster](#why-is-flashbang-faster).
...
| ------------------------------ | ------------: | --------------: | ------------- |
| Live worker, `!gh test` | **0.45 ms** | 0.48 ms | **Flashbang mean advantage 0.03 ms** (95% CI 0.02–0.04, p < 0.0001) |
| Live worker, full-catalog alias `!github test` | **0.46 ms** | 0.47 ms | **Flashbang mean advantage 0.02 ms** (95% CI 0.01–0.03, p = 0.0065) |
| Live worker, trailing `test !gh` | **0.46 ms** | 0.48 ms | **Flashbang mean advantage 0.04 ms** (95% CI 0.00–0.09, p = 0.0318) |
| Live worker, suffix `test gh!` | **0.46 ms** | 0.47 ms | **Flashbang mean advantage 0.02 ms** (95% CI 0.00–0.03, p = 0.0227) |
| Live worker, empty query `!gh` | **0.44 ms** | 0.47 ms | **Flashbang mean advantage 0.03 ms** (95% CI 0.02–0.05, p < 0.0001) |
| Live worker, 256-byte query | **0.46 ms** | 0.54 ms | **Flashbang mean advantage 0.08 ms** (95% CI 0.07–0.10, p < 0.0001) |
| Worker restart, `!gh test` | **2.94 ms** | 3.87 ms | **Flashbang won 60/60; mean advantage 0.93 ms** (95% CI 0.86–1.01, p < 0.0001) |
| Worker restart, full-catalog alias | **2.92 ms** | 3.85 ms | **Flashbang won 60/60; mean advantage 0.94 ms** (95% CI 0.86–1.01, p < 0.0001) |
| Worker restart, 256-byte query | **2.88 ms** | 4.08 ms | **Flashbang won 60/60; mean advantage 1.15 ms** (95% CI 1.08–1.22, p < 0.0001) |
| New profile, `!gh test` | **94.33 ms** | 146.67 ms | **Flashbang mean advantage 57.59 ms** (95% CI 39.51–75.68, p < 0.0001) |
| New profile, full-catalog alias | **93.92 ms** | 151.63 ms | **Flashbang mean advantage 62.69 ms** (95% CI 46.29–79.13, p < 0.0001) |

The comparison uses Unduckified's timing definition: CDP `Network.requestWillBeSent` wall time from navigation to the tool URL until the first GitHub request. It ran in Chromium 151 on an Apple M4 Max; discarded three warm-up pairs; randomized tool order; measured live workers, explicitly stopped workers, and isolated browser contexts; bootstrapped 95% confidence intervals; and used a paired sign-flip permutation test.

The payload audit separates critical-path behavior from total offline footprint. Flashbang's complete worker plus catalog measured 218.8 KiB transferred / 597.2 KiB decoded. Unduckified's build produces a smaller 191.0 KiB Brotli worker-plus-catalog artifact but a larger 693.9 KiB decoded runtime. During the final campaign, `s.dunkirk.sh` served `bangs.bin` with identity encoding, so its observed transfer was 690.6 KiB; that deployment variation is not used to claim an architectural payload win. Flashbang's first redirect needs only its cold module and, for a non-hot bang, one shard; its complete catalog continues caching after the destination request starts.
...
For a first-ever search, Flashbang starts its cold resolver module (~6.5 KiB transferred / ~20.7 KiB decoded). Generated hot bangs such as `!gh` resolve from the module's embedded table without fetching a catalog shard. Other built-ins such as `!github` start a deterministic routed shard in parallel with the module; across the current deployed 43-shard catalog, each shard is ~6.2–7.0 KiB transferred / ~13.8–15.8 KiB decoded. Flashbang redirects as soon as the required state resolves while Service Worker registration proceeds independently. Before registering, the page creates an empty build-specific handoff cache; activation consumes that signal and starts the complete catalog only after yielding to the destination. Controlled hot redirects schedule the same post-response warm under `FetchEvent.waitUntil()`. The full lookup is therefore available offline after the first successful use without competing with the first redirect, and rich fallback paths can still hand an already-loaded catalog and settings directly to the worker.
