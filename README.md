...
| **Redirect runtime payload¶**    | 🥈 **~220 KiB transfer / ~595 KiB decoded**                                                                                                           | 🥉 ~466 KiB transfer / ~1,980 KiB decoded                                                                                                        | 🥇 **~190 KiB transfer / ~686 KiB decoded**                                                                                                                               | 🥇 N/A                                                                                                                                             | ~525 KiB transfer / ~2,295 KiB decoded                                                                                                                         | 🥇 N/A                                                                                                                                                        |
...
‖ Counts are built-in redirect triggers after source aliases and project-specific entries are expanded. Flashbang's `data/bangs.json` contains 14,614 triggers; Unduckified's pack input at commit `fe47eeb` produces 13,593. Flashbang therefore covers 1,021 more triggers (7.5%), while Unduckified's measured Brotli runtime payload is 30 KiB (about 14%) smaller.

¶ Redirect runtime payload is the minified client code plus catalog data needed to resolve a built-in bang. Unduckified was remeasured live on August 2, 2026; Flashbang uses the current Cloudflare Pages build output, and the remaining deployments were measured live on July 23, 2026. The first value is the response-body bytes transferred using each deployment's supported content encoding; the second is the decoded size. Service Worker tools include their installed worker and packed redirect index. Page-based tools include the JavaScript and fetched data used by their redirect path. HTML, CSS, fonts, analytics, settings, suggestions, and other non-redirect assets are excluded. ReBang and cf-unduck return an empty-body edge `302` for a supported built-in bang, so their successful edge path has no client runtime payload; their larger client fallback paths are not included.

> **Note:** Unduckified was rechecked at current `main` commit `fe47eeb` on August 2, 2026; the remaining feature data was checked on July 23, 2026. "No" means no built-in implementation was found; manual browser configuration is not counted. These projects are actively developed and may have changed since.

### Flashbang vs Unduckified: measured lifecycle latency

In the production campaign below, Flashbang won **all 11 measured categories** under the benchmark's predeclared per-category 5% significance rule: six live-worker scenarios, three worker-restart scenarios, and two first-search scenarios. The live-worker advantage is statistically repeatable but only 0.02–0.10 ms in the representative rows below. The larger and more useful difference is the state Flashbang's hot boot is designed for: an **installed, cached profile whose Service Worker has been stopped by the browser**. On the next search, Unduckified reads and initializes its complete cached catalog before resolving the bang. Flashbang restores compact redirect state while the full catalog and IndexedDB state warm in the background.

| Chromium 151, randomized pairs | **flashbang** | **Unduckified** | Paired result |
| --- | ---: | ---: | --- |
| Live worker, `!gh test` | **0.42 ms** | 0.46 ms | **Flashbang mean advantage 0.02 ms** (95% CI 0.01–0.03, p < 0.0001) |
| Live worker, full-catalog alias `!github test` | **0.42 ms** | 0.45 ms | **Flashbang mean advantage 0.02 ms** (95% CI 0.02–0.03, p < 0.0001) |
| Live worker, 256-byte query | **0.43 ms** | 0.53 ms | **Flashbang mean advantage 0.10 ms** (95% CI 0.08–0.11, p < 0.0001) |
| Worker restart, `!gh test` | **2.86 ms** | 3.84 ms | **Flashbang won 60/60; mean advantage 0.99 ms** (95% CI 0.94–1.04, p < 0.0001) |
| Worker restart, full-catalog alias | **2.90 ms** | 3.92 ms | **Flashbang won 59/60; mean advantage 1.03 ms** (95% CI 0.93–1.12, p < 0.0001) |
| Worker restart, 256-byte query | **2.95 ms** | 4.08 ms | **Flashbang won 60/60; mean advantage 1.18 ms** (95% CI 1.11–1.25, p < 0.0001) |
| New profile, `!gh test` (60 pairs) | **126.93 ms** | 159.60 ms | **Flashbang mean advantage 29.73 ms** (95% CI 11.88–47.30, p = 0.0021) |
| New profile, full-catalog alias (60 pairs) | **123.02 ms** | 179.34 ms | **Flashbang mean advantage 40.64 ms** (95% CI 23.15–57.80, p < 0.0001) |

These numbers were measured on August 2, 2026 against `flashbang.tech` and Unduckified's public `s.dunkirk.sh` deployment, which was serving the v8 catalog format introduced by current `main` commit [`fe47eeb`](https://github.com/taciturnaxolotl/unduckified/commit/fe47eebbd25013bf454814b79c7e59a5d6bbb563). Unduckified's [checked-in benchmark](https://github.com/taciturnaxolotl/unduckified/blob/fe47eebbd25013bf454814b79c7e59a5d6bbb563/bench/redirect-bench.mjs) still points to an older preview serving v7, so that stale preview was not used for this table. The comparison otherwise uses Unduckified's timing definition—CDP `Network.requestWillBeSent` wall time from the tool navigation to the first GitHub request. It ran 60 measured pairs per category in Chromium 151 on an Apple M4 Max; measured live workers, explicitly stopped workers, and isolated new browser contexts; covered leading, trailing, suffix, bare, long-query, hot-tier, and full-catalog inputs; discarded three warm-up pairs; randomized tool order; bootstrapped 95% confidence intervals; and ran a paired sign-flip permutation test.

For a first-ever search, Flashbang starts the selected deterministic ~15–18 KiB catalog shard and the cold resolver module in parallel, redirects as soon as that shard resolves, then lets the Service Worker warm the complete catalog for later offline redirects. That new-profile state is a network-sensitive end-to-end measurement—CDN delivery, first-page fallback, catalog transfer, parsing, and installation—not a Service Worker restart measurement. The deployed campaign reversed the earlier cold result in Flashbang's favor for both the hot `!gh` query and the full-catalog `!github` alias. Unduckified does not publish the raw samples or environment behind its earlier claimed cold `p < 0.001`, so that exact claim cannot be audited.
...
The packed bang database (currently ~536 KiB uncompressed), settings UI, and suggestion index are separate artifacts, so a redirect does not parse metadata it does not need. First-page redirects select one of 16 content-hashed catalog shards from the trigger hash while the complete database warms after navigation. The raw parser, minimal-perfect-hash lookup, hot-boot tier, and deferred persistence behind that path are described in [How it works](#how-it-works).
