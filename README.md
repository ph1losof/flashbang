...
| **Redirect runtime payload¶**    | 🥈 **~217 KiB transfer / ~595 KiB decoded**                                                                                                           | 🥉 ~466 KiB transfer / ~1,980 KiB decoded                                                                                                        | 🥇 **~195 KiB transfer / ~694 KiB decoded**                                                                                                                               | 🥇 N/A                                                                                                                                             | ~525 KiB transfer / ~2,295 KiB decoded                                                                                                                         | 🥇 N/A                                                                                                                                                        |
...
> **Note:** Unduckified was rechecked at current `main` commit `00ea760` on August 2, 2026; the remaining feature data was checked on July 23, 2026. "No" means no built-in implementation was found; manual browser configuration is not counted. These projects are actively developed and may have changed since.

### Flashbang vs Unduckified: measured lifecycle latency

In the production campaign below, Flashbang was faster in all six live-worker scenarios and all three worker-restart scenarios. The live-worker advantage is statistically repeatable but only 0.02–0.09 ms in absolute terms. The larger and more useful difference is the state Flashbang's hot boot is designed for: an **installed, cached profile whose Service Worker has been stopped by the browser**. On the next search, Unduckified reads and initializes its complete cached catalog before resolving the bang. Flashbang restores compact redirect state and can resolve its 24 generated hot bangs—including `!gh`—while the full catalog and IndexedDB state warm in the background.

| Chromium 151, randomized pairs | **flashbang** | **Unduckified** | Paired result |
| --- | ---: | ---: | --- |
| Live worker, `!gh test` | **0.45 ms** | 0.47 ms | **Flashbang mean advantage 0.03 ms** (95% CI 0.02–0.04, p < 0.0001) |
| Live worker, full-catalog alias `!github test` | **0.42 ms** | 0.45 ms | **Flashbang mean advantage 0.03 ms** (95% CI 0.02–0.04, p < 0.0001) |
| Live worker, 256-byte query | **0.44 ms** | 0.54 ms | **Flashbang mean advantage 0.09 ms** (95% CI 0.08–0.11, p < 0.0001) |
| Worker restart, `!gh test` | **2.86 ms** | 3.98 ms | **Flashbang won 60/60; mean advantage 1.07 ms** (95% CI 0.99–1.14, p < 0.0001) |
| Worker restart, full-catalog alias | **2.85 ms** | 3.89 ms | **Flashbang won 60/60; mean advantage 1.08 ms** (95% CI 1.01–1.16, p < 0.0001) |
| Worker restart, 256-byte query | **2.90 ms** | 4.17 ms | **Flashbang won 60/60; mean advantage 1.25 ms** (95% CI 1.20–1.30, p < 0.0001) |
| New profile, `!gh test` (30 pairs) | 203.70 ms | **147.33 ms** | **Unduckified mean advantage 41.02 ms** (95% CI 14.58–66.47, p = 0.0062) |
| New profile, full-catalog alias (30 pairs) | 162.15 ms | 156.48 ms | Inconclusive: paired Unduckified − Flashbang 95% CI −43.64 to +15.85 ms (p = 0.3844) |

These numbers were measured on August 2, 2026 against Flashbang's production deployment and the same Unduckified preview URL used by [its own benchmark](https://github.com/taciturnaxolotl/unduckified/blob/8bbf72e808ec62d65549ee455e77e19e879b792e/bench/redirect-bench.mjs). The comparison uses Unduckified's timing definition—CDP `Network.requestWillBeSent` wall time from the tool navigation to the first GitHub request. It measures live workers, explicitly stopped workers, and isolated new browser contexts; covers leading, trailing, suffix, bare, long-query, hot-tier, and full-catalog inputs; discards three warm-up pairs; randomizes tool order; bootstraps 95% confidence intervals; and runs a paired sign-flip permutation test.

That production campaign predates the current source tree's fingerprint catalog and first-page hash shards. A first-ever non-hot search now downloads one deterministic ~15–18 KiB shard on its critical path; the Service Worker warms the complete catalog after navigation so later offline redirects retain the full catalog. The production table remains above until the optimized build is deployed and remeasured.

In a controlled current-source run with both projects served by the same local server, 20 paired live/restart rounds and 60 paired new-profile rounds produced **6 Flashbang wins, 0 Unduckified wins, and 5 statistical ties**. Live `!gh` and full-catalog lookups tied; Flashbang won the 256-byte live query by 0.08 ms, all three worker-restart categories by 1.17–1.23 ms, the new-profile hot query by 0.47 ms, and the new-profile `!github` query by 0.23 ms. Every reported win had p < 0.0001. These local results isolate implementation cost; they are not substituted into the production/CDN table above.

The new-profile row is not a Service Worker restart measurement: it combines CDN delivery, the first page fallback, catalog transfer, parsing, and installation. Its direction was also unstable across campaigns: a separate 60-pair reproduction of Unduckified's original two-category script found an 18.85 ms mean advantage for Unduckified, but its 95% CI crossed zero and the permutation result was p = 0.0578. Unduckified does not publish the raw samples or environment behind its claimed cold `p < 0.001`, so that exact claim cannot be audited.
